...
uint8_t crc8_dallas(const uint8_t* data, size_t len) {
  ...
  for (size_t i = 0; i < len; ++i) {
    ...
    for (uint8_t j = 0; j < 8; j++) {
      ...
      crc >>= 1;
      if (mix) crc ^= 0x8C; // poly 0x31 reflektionsform
      inbyte >>= 1;
    }
  }
  return crc;
...
void makeHeader(FrameHeader& h, uint8_t version, uint8_t type, uint16_t seq, uint16_t flags) {
  ...
  h.type    = type;
  h.seq     = seq;
  h.flags   = flags; // bit0 = CRC8 present
}
...
size_t encodeSensorV1(const FrameHeader& h, const SensorV1& s, uint8_t out[SENSOR_V1_TOTAL]) {
  ...
  for (int i = 0; i < 8; ++i) wr16LE(&b[22 + i*2], s.soil[i]); // 22..37

  // CRC8 över body (utan CRC-fältet och padding)
  uint8_t crc = crc8_dallas(b, SENSOR_V1_BODY_SZ - 2); // sista 2 bytes = crc & pad
  b[38] = crc;
  b[39] = 0; // pad
...
bool decodeSensorV1(const uint8_t in[SENSOR_V1_TOTAL], FrameHeader& h_out, SensorV1& s_out) {
  ...
  h_out.version = in[2];
  h_out.type    = in[3];
  h_out.seq     = rd16LE(&in[4]);
  h_out.flags   = rd16LE(&in[6]);
